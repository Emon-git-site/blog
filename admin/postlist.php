					...
					if ($post) {
						...
						while ($result = $post->fetch_assoc()) {
							...
							<tr class="odd gradeX">
								...
								<td>
						<?php
						   if(Session::get('userId') == $result['user_id'] || Session::get('userRole') == 0){	?>
								 <a href="editpost.php?editPostId=<?= $result['id'] ?>">Edit</a> ||
									<a href="deletePost.php?deletePostId=<?= $result['id'] ?>" onclick="return confirm('Are you sure want to delete?') ">Delete</a>  
									<?php  }else{ ?>
